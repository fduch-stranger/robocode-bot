...
class MinimumRiskMovementTest(unittest.TestCase):
    ...
    def test_confirmed_wave_pass_records_occupancy_without_hit_evidence(self) -> None:
        movement = MovementFlattener()
        bot = _bot(x=500.0, y=210.0, arena_width=1000.0, arena_height=1000.0, turn_number=20)
        wave = self._incoming_wave(1)
        movement._waves.append(wave)

        visits = movement.update(bot)

        center_bin = movement.config.bin_count // 2
        self.assertEqual(1, len(visits))
        self.assertAlmostEqual(1.0, movement._profile[(1, wave.distance_bucket, center_bin)])
        self.assertAlmostEqual(
            1.0,
            movement._visit_occupancy_profile.profile[(1, wave.distance_bucket, center_bin)],
        )
        self.assertEqual({}, movement._enemy_hit_profile.profile)
        self.assertEqual("occupancy", visits[0].evidence_kind)

    def test_expected_wave_pass_remains_only_in_legacy_behavior_profile(self) -> None:
        movement = MovementFlattener()
        bot = _bot(x=500.0, y=210.0, arena_width=1000.0, arena_height=1000.0, turn_number=20)
        wave = self._incoming_wave(1)
        wave.kind = "expected"
        wave.expected_confidence = 0.8
        movement._waves.append(wave)
        ...
        center_bin = movement.config.bin_count // 2
        self.assertAlmostEqual(1.0, movement._profile[(1, wave.distance_bucket, center_bin)])
        self.assertEqual({}, movement._visit_occupancy_profile.profile)
        self.assertEqual({}, movement._enemy_hit_profile.profile)
        self.assertEqual("expected_expired", visits[0].evidence_kind)
        self.assertEqual(0.0, visits[0].occupancy_visits)

    def test_real_hit_records_hit_profile_without_occupancy(self) -> None:
        movement = MovementFlattener()
        bot = _bot(x=500.0, y=210.0, arena_width=1000.0, arena_height=1000.0, turn_number=20)
        wave = self._incoming_wave(1)
        movement._waves.append(wave)

        visit = movement.record_bullet_hit(bot, target_id=1, bullet_power=3.0)

        self.assertIsNotNone(visit)
        assert visit is not None
        center_bin = movement.config.bin_count // 2
        self.assertAlmostEqual(3.0, movement._profile[(1, wave.distance_bucket, center_bin)])
        self.assertEqual({}, movement._visit_occupancy_profile.profile)
        self.assertAlmostEqual(
            1.0,
            movement._enemy_hit_profile.profile[(1, wave.distance_bucket, center_bin)],
        )
        self.assertEqual("enemy_hit", visit.evidence_kind)
        self.assertAlmostEqual(1.0, visit.hit_profile_support)
        self.assertAlmostEqual(0.0, visit.match_error)

    def test_expected_pressure_is_transient(self) -> None:
        movement = MovementFlattener()
        wave = self._incoming_wave(1)
        wave.kind = "expected"
        wave.expected_confidence = 0.8
        ...
        active = movement._evidence_breakdown(wave, center_bin, 500.0, 210.0)
        movement._waves.clear()
        expired = movement._evidence_breakdown(wave, center_bin, 500.0, 210.0)

        self.assertGreater(active.expected_pressure, 0.0)
        self.assertEqual(0.0, expired.expected_pressure)

    def test_split_evidence_direction_remains_shadow_only(self) -> None:
        bot = _bot(
            x=500.0,
            y=500.0,
            direction=90.0,
        ...
        movement = MovementFlattener()
        movement._waves.append(self._incoming_wave(1))
        with patch.object(movement, "_candidate_score", side_effect=(5.0, 1.0)):
            decision = movement.choose_direction(
                bot,
                body_bearing=0.0,
                strafe_offset=90.0,
                move_speed=8.0,
        ...
        self.assertEqual(-1, decision.direction)
        self.assertEqual(-1, decision.legacy_direction)
        self.assertEqual(1, decision.shadow_direction)
        self.assertEqual("legacy", decision.score_source)
        self.assertAlmostEqual(decision.current_count, decision.selected_current_danger)
        self.assertAlmostEqual(decision.alternative_count, decision.selected_alternative_danger)

    def test_split_evidence_go_to_is_reported_but_never_selected(self) -> None:
        movement = MovementFlattener()
        bot = _bot(
            x=500.0,
            y=500.0,
        ...
        decision = movement.choose_go_to_surf_destination(bot, target, 8.0, 80.0)

        assert decision is not None
        self.assertEqual("legacy", decision.score_source)
        self.assertEqual((decision.x, decision.y), (decision.live_x, decision.live_y))
        self.assertIsNotNone(decision.shadow_x)
        self.assertIsNotNone(decision.shadow_y)
        self.assertIsNotNone(decision.shadow_selected_danger)
    ...
    def test_bullet_shadow_can_use_event_bullet_source(self) -> None:
        ...
        bot = _bot(x=700.0, y=700.0, arena_width=1000.0, arena_height=1000.0, turn_number=1)
        wave = MovementWave(
            target_id=1,
            source_x=100.0,
            source_y=100.0,
        ...
        unshadowed = movement._danger(wave, center_bin, bot)
        movement.record_shadow_bullet_state("b1", 420.0, 100.0, 180.0, 14.0, 1)
        shadowed = movement._danger(wave, center_bin, bot)
