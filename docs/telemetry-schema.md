...
| `gun.eval_wave_visit` | - | `target`, `guess_factor`, `samples`, `traveled`, `distance`, `selected_gun`, `virtual_scores`, `gun_scores`, `traditional_gf_guess_factor`, `traditional_gf_error`, `traditional_gf_abs_error`, `traditional_gf_source`, `linear_wall_aware_wall_hit`, `linear_wall_aware_ticks`, `linear_wall_aware_final_speed`, `linear_accel_damped_effective_acceleration`, `linear_accel_damped_applied_ticks`, `linear_accel_damped_velocity_change_age`, `linear_accel_damped_final_speed` | `aim_mode` from `selected_gun`<br>`gun_mode` from `selected_gun` |
