...
- Adaptive Traditional GF keeps exact segments unchanged and uses a fixed coarse key of distance, lateral speed, and wall margin. Alternate coarse-key env controls were removed after July 2026 BasicGFSurfer validation did not justify the extra tuning surface. Density-supported peak selection remains experiment-gated via `ROBOCODE_ADAPTIVE_TRADITIONAL_GF_PEAK_SELECTION=density`; the default remains strongest-bin peak selection until validation justifies a promotion.
- A July 2026 BasicGFSurfer 12-round matrix found no default promotion. Forced `distance_lateral_velocity_age` had the best Traditional GF hit rate/error, `distance_lateral_advancing` and density scored well in forced mode, but normal selector results stayed mixed and density did not fire Traditional GF in its normal run. Use `tools/gun_eval_summary.py` source-level real hit, profile diagnostics, and GF error summaries before changing source penalties.
